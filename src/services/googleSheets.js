...
const exportAllDataToSheets = async (allData) => {
  ...
  const exportsConfig = [
    ...
    {
      title: 'Barang Bawaan',
      headers: ['ID', 'Nama Barang', 'Kategori', 'Tipe', 'Qty/Jumlah', 'Keterangan/Note', 'Status Checklist'],
      rows: allData.goods.map(g => [g._id.toString(), g.name, g.category, g.type, g.qty || '-', g.note || '-', g.isChecked ? 'Bawa' : 'Belum'])
    },
    {
      title: 'Joblist',
